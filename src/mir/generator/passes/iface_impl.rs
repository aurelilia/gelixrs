...
fn iface_impl(gen: &mut MIRGenerator, iface_impl: &mut IFaceImpl) -> Res<()> {
    ...
    add_generic_aliases(gen, &iface.generics, &iface_impl.iface_generics, &iface_impl.iface)?;

    for method in iface_impl.methods.iter_mut() {
        let iface_method = iface.methods.get(&method.sig.name.lexeme).or_err(gen, &method.sig.name, "Method is not defined in interface.")?;
...
fn add_generic_aliases(gen: &mut MIRGenerator, iface: &Vec<Rc<String>>, impl_: &Vec<Token>, iface_tok: &Token) -> Res<()> {
    if iface.len() != impl_.len() {
        return Err(gen.error(
            iface_tok,
            iface_tok,
            &format!("Wrong amount of interface generic parameters (expected {}; got {})", iface.len(), impl_.len()),
        ))
    }
...
fn check_equal_signature(gen: &mut MIRGenerator, method: &Function, mir_method: Rc<MIRVariable>, iface_method: &MIRIFaceMethod) -> Res<()> {
    let mir_method = if let MIRType::Function(method) = &mir_method.type_ { method } else { panic!() };
