...
impl ModulePass for Generate {
    ...
    fn run_mod(&mut self, module: MutRc<MModule>) -> Result<(), Vec<Error>> {
        ...
        let mut types: Vec<Type> = module.borrow().types.values().cloned().collect();
        ...
        for ty in types {
            match ty {
                Type::Function(func) => {
                    ...
                    gen.generate_function(&ast, None).map_err(|e| errs.push(e)).ok();
                }

                Type::Class(class) => {
                    let ast = Rc::clone(&class.borrow().ast);
                    gen.generate_constructors(&ast).map_err(|e| errs.push(e)).ok();

                    for method in ast.methods.iter() {
                        let mir = &class.borrow().methods[&method.sig.name.lexeme];
                        gen.generate_function(method, Some(mir.type_.as_function())).map_err(|e| errs.push(e)).ok();
                    }
                }
